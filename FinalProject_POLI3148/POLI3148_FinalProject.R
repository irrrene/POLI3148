...
d_ext_unnested <- d_ext |>
  select(Title,  topic, Title_tokenized, Text_tokenized) |>
  unnest(Text_tokenized) |>
  rename("word" = "Text_tokenized")

# Remove stop word using anti_join
d_ext_remove_sw <- d_ext_unnested |>
  anti_join(baidu_stopwords4topic, by = c("word" = "V1"))

d_ext_remove_sw <- d_ext_remove_sw %>% filter(!str_detect(word, "^\\s*[0-9]*\\s*$"))
d_ext_remove_sw <- d_ext_remove_sw %>% filter(!str_detect(word, "-"))
d_ext_remove_sw <- d_ext_remove_sw %>% filter(!str_detect(word, "\\."))
...
career_freq <- d_ext_remove_sw |> 
  filter(topic == "career") |>
  group_by(word) |>
  count() |>
  arrange(-n)

disable_freq <- d_ext_remove_sw |> 
  filter(topic == "disable") |>
  group_by(word) |>
  count() |>
  arrange(-n)

repro_freq <- d_ext_remove_sw |> 
  filter(topic == "maternal") |>
  group_by(word) |>
  count() |>
  arrange(-n)

health_freq <- d_ext_remove_sw |> 
  filter(topic == "health") |>
  group_by(word) |>
  count() |>
  arrange(-n)

mental_freq <- d_ext_remove_sw |> 
  filter(topic == "mental") |>
  group_by(word) |>
  count() |>
  arrange(-n)

edu_freq <- d_ext_remove_sw |> 
  filter(topic == "education") |>
  group_by(word) |>
  count() |>
  arrange(-n)

right_freq <- d_ext_remove_sw |> 
  filter(topic == "rights") |>
  group_by(word) |>
  count() |>
  arrange(-n)
...
#wordcloud(career_freq$word, word_freq$n, min.freq = 300)
...
ggplot(nvxing_topicplot, aes(x = type, y = AmountLog)) +
  geom_boxplot(outlier.shape = NA, color = "cyan3") +
  geom_dotplot(binaxis = "y", stackdir = "center", dotsize = 0.2, alpha = 0.5) + 
...
ggplot(nvxing_topicplot, aes(x = type, y = RaisedAmount, fill = topic)) +
  ...
  theme(legend.position = "right",
        axis.text.x = element_text(angle = 45, hjust = 1))
# mosaic plot - just testing, not that good
library(ggmosaic)
ggplot(data = nvxing) +
  geom_mosaic(aes(x = product(type), fill=topic, weight = RaisedAmount)) +
  theme_mosaic() +
  scale_fill_tableau()
...
# Just testing using geom_bin_2d here
ggplot(data = nvxing) +
  geom_bin_2d(aes(x = type, y = topic)) +
